...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_category() ) {
		...
		echo dh_print_string($prepend) . dh_print_string($before) .'<span typeof="v:Breadcrumb" property="v:title">'. esc_html__( 'Products brands &ldquo;', 'woow' ) . $queried_object->name . '&rdquo;' .'</span>'. dh_print_string($after);
