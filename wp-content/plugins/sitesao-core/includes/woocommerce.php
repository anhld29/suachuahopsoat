...
if ( ! class_exists( 'DH_Woocommerce' ) ) :
	class DH_Woocommerce {
		...
		public function init() {
			...
			if(class_exists('WC_Deprecated_Filter_Hooks'))
		...
		public function custom_tab_content(){
			...
			echo do_shortcode(dh_get_post_meta('custom_tab_content',$product->get_id(),$deafult_custom_tab_content));
		}
		...
		public function single_fullwidth_layout(){
			...
			if(is_product() ){
				$attachment_ids = wc_get_product(get_the_ID())->get_gallery_image_ids();
		...
		protected function _get_minicart_mobile() {
			...
			$cart_output = '<a href="' . wc_get_cart_url() . '" title="' . __( 'View Cart', 'sitesao' ) . '"  class="cart-icon-mobile">'.$this->_get_minicart_icon2().' '.( ! empty( $cart_count ) ? '<span>' . $cart_count . '</span>' : '' ) . '</a>';
			return $cart_output;
		}
				...
				if (! empty( $cart_count ) ) {
					...
					foreach ( $woocommerce->cart->cart_contents as $cart_item_key => $cart_item ) {
						...
						if ( $cart_product->exists() && $cart_item['quantity'] > 0 ) {
							...
							$minicart .= '<div class="cart-product-quantity-price">' . $cart_item['quantity'] . ' x ' .WC()->cart->get_product_price($cart_product)  . '</div>';
					...
					$minicart .= '<a class="viewcart-button button" href="' . esc_url( wc_get_cart_url() ) . '"><span class="text">' . __( 'View Cart', 'sitesao' ) . '</span></a>';
					$minicart .= '<a class="checkout-button button" href="' . esc_url( wc_get_checkout_url() ) . '"><span class="text">' . __( 'Checkout', 'sitesao' ) . '</span></a>';
		...
		public function template_loop_quickview() {
			...
			if(apply_filters('dh_woocommerce_quickview', true))
				echo '<div class="shop-loop-quickview"><a data-product_id ="' . $product->get_id() . '" title="' .esc_attr__( 'Quick view', 'sitesao' ) . '" href="' . esc_url( $product->get_permalink() ) . '">'. __( 'Quick view', 'sitesao' ) .'</a></div>';
		}
